...
export function brandedEmailHtml({
  ...
  const rows = details
    ...
    .map(
      ([label, value]) =>
        `<tr><td style="padding:9px 0;color:#64748b;font-size:12px;width:42%;vertical-align:top;">${escapeHtml(label)}</td><td style="padding:9px 0;color:#0a1931;font-size:13px;font-weight:600;vertical-align:top;">${escapeHtml(value)}</td></tr>`,
    )
    .join("");
  ...
  const itemsHtml =
    Array.isArray(items) && items.length > 0
      ? `
    <div style="border-top:1px solid #e2e8f0;margin:20px 0;padding-top:16px;">
      <h2 style="margin:0 0 14px;font-size:13px;color:#475569;text-transform:uppercase;letter-spacing:.06em;">Selected Products (${items.length})</h2>
      <table role="presentation" style="width:100%;border-collapse:collapse;">
        ${items
          .map(
            (item) => `
          <tr style="border-bottom:1px solid #f1f5f9;">
            ${item.image ? `<td style="padding:10px 12px 10px 0;width:56px;vertical-align:middle;"><img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.name || "Product")}" style="width:52px;height:52px;object-fit:cover;border-radius:8px;border:1px solid #e2e8f0;display:block;" /></td>` : ""}
            <td style="padding:10px 0;vertical-align:middle;">
              <div style="color:#0a1931;font-size:13px;font-weight:700;line-height:1.4;">${escapeHtml(item.name || "Product item")}</div>
              <div style="color:#64748b;font-size:12px;margin-top:3px;">Quantity: <strong style="color:#0b56d9;">${escapeHtml(item.quantity || 1)}</strong></div>
            </td>
          </tr>
        `,
          )
...
export async function sendConfirmationEmail({
  ...
  if (!mailToken) {
    console.warn(
      "sendConfirmationEmail skipped: missing ZEPTO_TOKEN / ZEPTOMAIL_TOKEN env variable",
    );
    return false;
  }
  ...
  try {
    ...
    console.log(
      `Confirmation email sent successfully to ${email} for reference ${code}`,
    );
    return true;
  } catch (error) {
...
export function whatsappUrl(message) {
  const number = (
    process.env.WHATSAPP_BUSINESS_NUMBER || "919790361222"
  ).replace(/\D/g, "");
