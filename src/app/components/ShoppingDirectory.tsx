    ...
    <section id="shop-directory" className="scroll-mt-24 border-b border-slate-200 bg-[#F7F9FF] py-14 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-8">
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          ...
          {activeTab === 'directory' && <div className="flex flex-wrap items-center gap-3"><div className="relative"><select value={selectedCategory} onChange={(event) => setSelectedCategory(event.target.value)} className="w-full cursor-pointer appearance-none rounded-full border border-slate-300 bg-white py-2.5 pl-4 pr-10 text-xs font-bold text-[#0A1931] focus:outline-none focus:ring-2 focus:ring-[#0B56D9]"><option value="">All Categories</option>{regularCategories.map((category) => <option key={category.categoryID} value={category.categoryID}>{category.categoryName}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /></div><span className="whitespace-nowrap rounded-full border border-slate-300 bg-slate-200 px-3.5 py-2 text-[11px] font-bold text-[#0A1931]">{directoryProducts.length} items</span>{(selectedCategory || searchQuery) && <button type="button" onClick={clearDirectoryFilters} className="cursor-pointer text-xs font-extrabold text-[#0B56D9] hover:underline">Clear filters</button>}</div>}
        </div>
